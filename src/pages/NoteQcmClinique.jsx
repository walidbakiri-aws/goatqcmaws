...
function NoteQcmClinique(props) {
  ...
  const AjouterImage = async (qcmId) => {
    ...
    formData.append("qcmClinique", JSON.stringify(result.data));
    formData.append("OurUsers", JSON.stringify(saveUserQcm));
    axios
      .post("https://goatqcm-instance.com/noteqcmclinique/uploadimage", formData, {
        headers: {
  ...
  const AjouterDesc = async (qcmId) => {
    ...
    formData.append("OurUsers", JSON.stringify(saveUserQcm));
    axios
      .post("https://goatqcm-instance.com/noteqcmclinique/uploanoteqcm", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      })
  ...
  const testDescExsite = async (qcmId) => {
    ...
    try {
      const fullDescResultiniial = await axios.get(
        `https://goatqcm-instance.com/noteqcmclinique/${qcmId}/${userId}`
      );
      console.log(fullDescResultiniial.data.id);
      getResultNote.value = await axios.get(
        `https://goatqcm-instance.com/noteqcmclinique/${fullDescResultiniial.data.id}`
      );
      console.log(getResultNote.value.data);
    } catch (Exception) {}
    ...
    if (getResultNote.value) {
      try {
        ...
        setFileDisplayEdite(
          `https://goatqcm-instance.com/noteqcmclinique/image/${getResultNote.value.data.id}/${getResultNote.value.data.imageName}`
        );
  ...
  const UpdateImage = async (qcmId) => {
    ...
    await axios
      .put(
        `https://goatqcm-instance.com/noteqcmclinique/updateimage/${getResultNote.value.data.id}`,
        formData
      )
      .then((res) => {
  ...
  const UpdateDesc = async (qcmId) => {
    console.log("update item");
    const formData = new FormData();
    formData.append("noteqcm", noteQcmEdite.noteQcmClinique);
    await axios
      .put(
        `https://goatqcm-instance.com/noteqcmclinique/${getResultNote.value.data.id}`,
        formData
      )
      .then((res) => {
        console.log("success updating");
          ...
          <div className={classes.fulldescription}>
            ...
            <div className={classes.descarea}>
              <textarea
                className="form-control"
                id="exampleFormControlTextarea1"
                value={noteQcmEdite.noteQcmClinique}
                onChange={(e) =>
                  setNoteQcmEdite({
                    ...noteQcmEdite,
                    noteQcmClinique: e.target.value,
                  })
                }
              ></textarea>
            </div>
      ...
      {isTabletOrMobile && VisisbleDescUpdate && (
        <div className={classes.imgdescdiv_phone}>
          <div className={classes.fulldescription_phone}>
            ...
            <button
              type="button"
              className={`${classes.deeletefulldesc_clinique_phone} btn btn-danger`}
              onClick={(e) => deleteFullDesc(props.qcmId)}
            >
              Delete
            </button>
...
export default NoteQcmClinique;
